...
class ChatArea extends StatelessWidget {
    ...
    return InkWell(
      ...
      child: Container(
        ...
        foregroundDecoration: BoxDecoration(
          color: isMsgSelected ? ColorRes.darkOrange.withOpacity(0.3) : ColorRes.transparent,
  ...
  Widget textMessage(ChatMessage? data, {EdgeInsets? padding, bool isMsgSelected = false}) {
    bool isMe = data?.senderUser?.userid == PrefService.userId;
    return ConstrainedBox(
      constraints: BoxConstraints(maxWidth: data?.msgType == FirebaseRes.msg ? Get.width / 1.4 : Get.width / 1.7),
      child: Padding(
        padding: padding ?? const EdgeInsets.symmetric(vertical: 13.0, horizontal: 11),
        child: Text(
          data?.msg ?? '',
          style: TextStyle(color: isMe ? ColorRes.white : ColorRes.darkGrey, fontFamily: FontRes.regular),
        ),
      ),
    );
  }
  ...
  Widget imageMessage(ChatMessage? data, {required ChatScreenViewModel model}) {
    return Column(
      ...
      children: [
        imageVideoPosterWidget(data: data, onTap: model.timeStamp.isEmpty ? (data) => onImageTap(data) : null),
        if ((data?.msg ?? '').isNotEmpty)
          textMessage(data, padding: const EdgeInsets.only(bottom: 8.0, left: 10, right: 10)),
      ],
    );
  }
        ...
        Column(
          ...
          children: [
            imageVideoPosterWidget(
                data: data, onTap: model.timeStamp.isEmpty ? (data) => onVideoItemClick(data) : null),
            if ((data?.msg ?? '').isNotEmpty)
              textMessage(data, padding: const EdgeInsets.only(bottom: 8.0, left: 10, right: 10)),
          ],
        ),
