...
class ImageVideoSendSheet extends StatelessWidget {
  ...
  const ImageVideoSendSheet({Key? key, required this.image, required this.onSendBtnClick, required this.selectedItem})
      : super(key: key);
        ...
        children: [
              ...
              Expanded(
                child: ClipRRect(
                  ...
                  child: selectedItem == S.current.image
                      ...
                      : Image.file(File('$image'), height: 170, fit: BoxFit.cover),
                ),
              ),
              const SizedBox(width: 5),
              Expanded(
          ...
          InkWell(
            ...
            child: Container(
              height: 40,
              width: Get.width / 3,
              decoration: BoxDecoration(borderRadius: BorderRadius.circular(10), gradient: StyleRes.linearGradient),
              child: Center(
                child:
                    Text(S.current.send, style: const TextStyle(color: ColorRes.white, fontFamily: FontRes.semiBold)),
              ),
            ),
          )
        ],
