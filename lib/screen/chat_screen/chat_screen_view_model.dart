...
class ChatScreenViewModel extends BaseViewModel {
  ...
  void onAddBtnTap(int type) async {
    ...
    Get.bottomSheet(
        ItemSelectionDialogAndroid(
            onImageBtnClick: () => itemSelectImage(type), onVideoBtnClick: () => itemSelectVideo(type)),
    ...
    ApiProvider().getStoreFileGivePath(image: cameraImage).then(
      (value) {
        if (value.status == true) {
          Get.back();
          Get.bottomSheet(
              ImageVideoSendSheet(image: value.path, onSendBtnClick: onSendBtnClick, selectedItem: selectedItem),
              isScrollControlled: true);
        }
        notifyListeners();
      },
  ...
  void itemSelectVideo(int type) async {
    ...
    _pickedFile = await picker.pickVideo(
        source: type == 0 ? ImageSource.gallery : ImageSource.camera, maxDuration: const Duration(seconds: 60));
    ...
    if (sizeInMb <= 15) {
      VideoThumbnail.thumbnailFile(video: _pickedFile!.path).then(
        (value) {
          Get.back();
          Get.bottomSheet(
            ImageVideoSendSheet(
              ...
              onSendBtnClick: (msg, image) {
                ...
                ApiProvider().getStoreFileGivePath(image: File(image ?? '')).then(
                  (value) {
  ...
  void onVideoItemClick(ChatMessage? data) {
    Get.to(() => VideoPreviewScreen(videoUrl: data?.video, type: VideoType.other))?.then((value) {});
